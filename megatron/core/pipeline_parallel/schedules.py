...
import os
def _pp_schedule(config):
    return getattr(config, 'pipeline_schedule', None) or os.environ.get('PIPELINE_SCHEDULE', '1f1b')
_TRACE = []
def _trace(kind, mb, chunk):
    if os.environ.get('PP_TRACE'):
        _TRACE.append((parallel_state.get_pipeline_model_parallel_rank(), kind, int(mb), int(chunk or 0)))

import atexit, json
def _dump_trace():
    if os.environ.get('PP_TRACE'):
        try:
            r = parallel_state.get_pipeline_model_parallel_rank()
        except Exception:
            r = int(os.environ.get('RANK', 0))
        path = os.environ.get('PP_TRACE_DIR', os.getcwd())
        os.makedirs(path, exist_ok=True)
        with open(os.path.join(path, f"trace_rank{r}.json"), "w") as f:
            json.dump(_TRACE, f)
...
def forward_step(
    ...
    _trace('F', current_microbatch, vp_stage)
...
def forward_backward_pipelining_with_interleaving(
    *,
    ...
    if _pp_schedule(config) == 'looped_bfs':
    ...
    def backward_step_helper(virtual_microbatch_id):
        ...
        _trace('B', microbatch_id_table[virtual_microbatch_id % total_num_microbatches], model_chunk_id)
    ...
    *,
    ...
    if not forward_only:
        for i in range(num_warmup_microbatches):
            ...
            _trace('B', num_microbatches_remaining + i, 0)
